...
public class HondaActionTest {
 ...
 static class Fake extends HondaIntegrationTest.FakeService {
  ...
  boolean blockModeOn,blockModeOff,ackBeforeReply;
  CountDownLatch blocked,release;
  void block() {blocked.countDown();try{release.await(5,TimeUnit.SECONDS);}catch(InterruptedException e){Thread.currentThread().interrupt();}}

  @Override protected boolean onTransact(int code,Parcel data,Parcel reply,int flags)throws RemoteException{
   if(code==0x4f&&(blockModeOn||blockModeOff)){
    int position=data.dataPosition();data.enforceInterface(HondaProtocol.DESCRIPTOR);int mode=data.readInt();data.setDataPosition(position);
    if(mode==(blockModeOn?1:0))block();
   }
   if(code==0x54&&ackBeforeReply){try{result(1,0);}catch(Exception e){throw new RemoteException(e.toString());}block();}
 ...
 void ready()throws Exception{client.connect();await(()->client.phase==HondaClient.Phase.DISCOVERING&&fake.discovers>0);fake.categories(1,3);await(()->fake.reads==1);fake.values(TPMS);await(()->fake.reads==2);fake.values(TACH);await(()->client.phase==HondaClient.Phase.READY);}
 ...
 @Test public void resetRediscoverAllSettings()throws Exception{ready();client.resetCustomization(true);await(()->fake.resets==1);fake.result(0,0);await(()->client.phase==HondaClient.Phase.DISCOVERING&&fake.discovers==2);assertTrue(client.values.isEmpty());fake.categories(3);await(()->fake.reads==3);fake.values(TACH);await(()->client.phase==HondaClient.Phase.READY);assertTrue(client.status.contains("not independently verified"));}
 ...
 @Test public void acknowledgementBeforeActionDispatchIsIgnored()throws Exception{
  ready();fake.blockModeOn=true;fake.blocked=new CountDownLatch(1);fake.release=new CountDownLatch(1);
  client.calibrateTpms(true);assertTrue(fake.blocked.await(3,TimeUnit.SECONDS));
  fake.result(1,0);ShadowLooper.idleMainLooper();assertEquals(HondaClient.Phase.ACTION,client.phase);assertEquals(0,fake.tpms);
  fake.release.countDown();await(()->fake.tpms==1);assertEquals(HondaClient.Phase.ACTION,client.phase);
  fake.result(1,0);await(()->fake.reads==3);fake.values(TPMS);await(()->client.phase==HondaClient.Phase.READY);
 }
 @Test public void staleValuesBeforeReadDispatchCannotConfirmAction()throws Exception{
  ready();fake.blockModeOff=true;fake.blocked=new CountDownLatch(1);fake.release=new CountDownLatch(1);
  client.calibrateTpms(true);await(()->fake.tpms==1);fake.result(1,0);
  await(()->fake.blocked.getCount()==0);fake.values(TPMS);ShadowLooper.idleMainLooper();
  assertEquals(HondaClient.Phase.VERIFYING,client.phase);assertEquals(2,fake.reads);
  fake.release.countDown();await(()->fake.reads==3);fake.values(TPMS);await(()->client.phase==HondaClient.Phase.READY);
 }
 @Test public void callbackSuccessCannotOverrideRejectedRequestReturn()throws Exception{
  ready();fake.ackBeforeReply=true;fake.blocked=new CountDownLatch(1);fake.release=new CountDownLatch(1);
  client.calibrateTpms(true);assertTrue(fake.blocked.await(3,TimeUnit.SECONDS));ShadowLooper.idleMainLooper();
  assertEquals(HondaClient.Phase.ACTION,client.phase);assertEquals(2,fake.reads);
  fake.reject=true;fake.release.countDown();await(()->client.phase==HondaClient.Phase.DISCONNECTED);
  assertTrue(client.status.contains("rejected"));assertEquals(2,fake.reads);
 }
}
