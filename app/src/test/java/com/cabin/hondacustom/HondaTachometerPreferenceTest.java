...
public class HondaTachometerPreferenceTest {
 ...
 boolean bindingFailure,delayBinding,syncVehicleDisconnect,syncUnitDisconnect; ServiceConnection delayed; int unbound;
 static class Unit extends Binder {
  ...
  @Override public String getInterfaceDescriptor(){if(descriptorEntered!=null){descriptorEntered.countDown();try{descriptorRelease.await(5,TimeUnit.SECONDS);}catch(InterruptedException e){Thread.currentThread().interrupt();}}return UnitInfoProtocol.DESCRIPTOR;}
 ...
 @Before public void setup()throws Exception{unit=new Unit();vehicle=new HondaIntegrationTest.FakeService();entry=new Catalog(RuntimeEnvironment.getApplication()).byKey.get("3:92");
  Context context=new ContextWrapper(RuntimeEnvironment.getApplication()){
   @Override public boolean bindService(Intent i,ServiceConnection c,int flags){
    if(i.getComponent().getClassName().equals(HondaProtocol.SERVICE)){if(syncVehicleDisconnect)c.onServiceDisconnected(i.getComponent());else c.onServiceConnected(i.getComponent(),vehicle);return true;}
    assertEquals(UnitInfoProtocol.SERVICE,i.getComponent().getClassName());if(bindingFailure)return false;if(syncUnitDisconnect){c.onServiceDisconnected(i.getComponent());return true;}if(delayBinding){delayed=c;return true;}c.onServiceConnected(i.getComponent(),unit);return true;
   }
   @Override public void unbindService(ServiceConnection c){unbound++;}
  };client=new HondaClient(context,()->{});
 }
 @After public void close(){if(unit.descriptorRelease!=null)unit.descriptorRelease.countDown();client.destroy();}
 ...
 void ready()throws Exception{client.connect();await(()->client.phase==HondaClient.Phase.DISCOVERING&&vehicle.discovers>0);vehicle.categories(3);await(()->vehicle.reads==1);vehicle.values(HondaIntegrationTest.TACH);await(()->client.phase==HondaClient.Phase.READY);assertEquals(0,unit.writes);}
 ...
 @Test public void disconnectDuringPreferenceDescriptorLookupPreventsWrite()throws Exception{
  ready();unit.descriptorEntered=new CountDownLatch(1);unit.descriptorRelease=new CountDownLatch(1);
  client.changeSystemTachometer(entry,2,true);assertTrue(unit.descriptorEntered.await(3,TimeUnit.SECONDS));
  client.disconnect();unit.descriptorRelease.countDown();
  // Reconnect is serialized after the cancelled preference worker, so reaching discovery
  // proves that worker has completed rather than merely not writing yet.
  int discovered=vehicle.discovers;client.connect();await(()->vehicle.discovers>discovered);
  assertEquals(0,unit.writes);assertEquals(0,vehicle.changes);
  client.disconnect();assertFalse(client.status.contains("preference write was attempted"));
 }
 @Test public void synchronousVehicleDisconnectUnbindsReturnedBindingExactlyOnce(){
  syncVehicleDisconnect=true;client.connect();assertEquals(HondaClient.Phase.DISCONNECTED,client.phase);
  assertEquals(1,unbound);assertFalse(client.status.contains("NullPointerException"));client.disconnect();assertEquals(1,unbound);
 }
 @Test public void synchronousUnitDisconnectUnbindsBothReturnedBindings()throws Exception{
  ready();syncUnitDisconnect=true;client.changeSystemTachometer(entry,2,true);
  assertEquals(HondaClient.Phase.DISCONNECTED,client.phase);assertEquals(2,unbound);
  assertEquals(0,unit.writes);client.disconnect();assertEquals(2,unbound);
 }
 @Test public void currentCachedTachometerStillGetsFreshVehicleConfirmation()throws Exception{
  ready();client.changeSystemTachometer(entry,1,true);await(()->vehicle.changes==1);
  assertEquals(1,unit.stored);assertEquals(1,unit.reads);assertEquals(HondaClient.Phase.WRITING,client.phase);
  vehicle.result(3,0);await(()->vehicle.reads==2);vehicle.values(HondaIntegrationTest.TACH);
  await(()->client.phase==HondaClient.Phase.READY);assertTrue(client.status.contains("preference also verified"));
 }
}
