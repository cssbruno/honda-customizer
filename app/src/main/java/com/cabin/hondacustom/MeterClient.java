...
public final class MeterClient {
    ...
    private void send(Session active,Work work){
    ...
    public void connect(){
        ...
        note("Connecting to Honda instrument contents…");if(!current(active))return;arm();
        ...
        try{active.bound=context.bindService(intent,active,Context.BIND_AUTO_CREATE);
            // A synchronous bind callback can disconnect before bindService returns its registration.
            if(!current(active)){
                if(active.bound){active.bound=false;try{context.unbindService(active);}catch(IllegalArgumentException ignored){}}
                return;
            }
            if(!active.bound)fail("Honda meter service unavailable. This requires the original Honda ExternalDisplayApService.");
        }catch(Exception e){if(current(active))fail(message(e));}
    }
    public List<Integer> draft(){return Collections.unmodifiableList(new ArrayList<>(draft));}
    /** Latest reported editor capacity; loading defaults does not replace the verified baseline. */
    ...
    public boolean canAdd(int id){return editable()&&draft.size()<capacity&&available().contains(id)&&!draft.contains(id);}
    ...
    private void read(int requested){
        ...
        request=requested;phase=Phase.READING;note(requested==4?"Loading default contents…":"Reading Honda instrument contents…");
        if(active==null||!current(active))return;arm();
        send(active,s->{MeterContents.Capabilities found=s.protocol.capabilities();
    ...
    private void onData(int result,MeterContents data){
        ...
        if(phase==Phase.VERIFYING){
            ...
            pending=null;live=data;capacity=data.max;draft.clear();draft.addAll(data.contents());phase=Phase.READY;
            note("Instrument contents confirmed by Honda and verified by readback.");return;
        }
        if(request==MeterProtocol.REQUEST_DEFAULT){
            if(live==null||capabilities==null||!capabilities.permits(live,data.contents(),data.max)){fail("Default layout cannot safely replace this preset with the reported capabilities.");return;}
            capacity=data.max;draft.clear();draft.addAll(data.contents());phase=Phase.READY;note("Default contents loaded into draft. Save to apply to preset "+live.preset+".");return;
        }
        live=data;capacity=data.max;draft.clear();draft.addAll(data.contents());phase=Phase.READY;
    ...
    public void save(boolean parked){
        if(!parked||!editable()||!capabilities.permits(live,draft,capacity)){note("Save unavailable. Load a supported preset while parked and preserve required contents.");return;}
        ...
        Session active=session;
        phase=Phase.WRITING;note("Saving instrument contents; waiting for Honda confirmation…");
        if(!current(active))return;arm();
        send(active,s->{
            MeterContents.Capabilities fresh=s.protocol.capabilities();
            if(!fresh.permits(baseline,order,limit))throw new IllegalStateException("Honda content capabilities changed; reload before saving");
            if(!current(s))return;
            main.post(()->{if(current(s))capabilities=fresh;});
    ...
    private void onChanged(int result){
        ...
        final int preset=request;Session active=session;
        note("Honda acknowledged the save; verifying preset "+preset+"…");if(!current(active))return;arm();
        send(active,s->{s.protocol.mode(false);s.mode=false;if(current(s))s.protocol.read(preset);});
    }
    ...
    private void disconnectInternal(){
        disarm();Session old=session;session=null;++generation;pending=null;live=null;capacity=0;capabilities=null;draft.clear();phase=Phase.DISCONNECTED;
        if(old!=null){
